 ...
 */
	function chatgpt_assistant_enqueue_assets(): void
	{
		...
		$plugin_pages = array(
			...
			'ai-content-generator-marketing_page_chatgpt-assistant-upgrade',
			'ai-content-generator-marketing_page_chatgpt-assistant-new-post'
		);

		// Check if the current screen is in your plugin's pages
		if ($screen->id == 'ai-content-generator-marketing_page_chatgpt-assistant-messages') {
			// Enqueue Bootstrap CSS
			wp_enqueue_style('bootstrap', $plugin_directory . 'external/bootstrap.min.css', array(), '4.5.2');

			// Enqueue Bootstrap JS
			wp_enqueue_script('bootstrap', $plugin_directory . 'external/bootstrap.min.js', array('jquery'), '4.5.2');
		}
		...
		if (in_array($screen->id, $plugin_pages)) {
			// Enqueue Bootstrap CSS
			wp_enqueue_style('bootstrap', $plugin_directory . 'external/bootstrap.min.css', array(), '5.3.1');
			wp_enqueue_style('bs-stepper', $plugin_directory . 'external/bs-stepper.min.css');

			// Enqueue Bootstrap JS
			wp_enqueue_script('bootstrap', $plugin_directory . 'external/bootstrap.bundle.min.js', array(), '5.3.1');
			wp_enqueue_script('bs-stepper', $plugin_directory . 'external/bs-stepper.min.js');

			wp_enqueue_script('jquery');
			wp_enqueue_script('twbs', $plugin_directory . 'external/jquery.twbsPagination.min.js');

			wp_enqueue_script('fontawesome', $plugin_directory . 'external/all.min.js');
			wp_enqueue_style('fontawesome', $plugin_directory . 'external/all.min.css');
